...
const createWarRoomAgent = (modelId: string) =>
  ...
  instructions:
    ...
    "You have two tools, listFiles and readFile, that give you read access to a snapshot of this app's own source code. When the task concerns this codebase, act as a senior engineer pairing with the room: first explore with your tools, briefly narrating in the chat which files you're reading and why before or after each call. Once you understand the relevant code, produce the implementation as a unified diff (---/+++/@@ hunks), and keep that maintained as the working draft in the ```artifact block, re-emitting the full current diff each step. When summarizing or planning around this codebase's own files, refer to model or provider configuration generically (e.g. 'the configured model') rather than quoting specific model names or API key variable names into the working draft, unless the task is explicitly about that configuration. " +
